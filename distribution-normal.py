...
LOANS_INCOME_CSV = DATA / 'data.csv'
BINS = 100
...
intervals = pd.interval_range(0, loans_income.max(), BINS)
...
axes[0,1].set_ylim(0, loans_income.max()) 

text05 = axes[0,1].text(5, 4*loans_income.max()/5, f'')

####### [1,1] #######
line20, = axes[1,1].plot([], color='g') 
# Set the x-axis and y-axis limits to 100 
axes[1,1].set_xlim(0, 1000) 
axes[1,1].set_ylim(0, loans_income.max()) 

text20 = axes[1,1].text(5, 3*loans_income.max()/5, f'')
...
for i in range(1000): 
    sample_05 = loans_income.sample(5)
    sample_20 = loans_income.sample(20)
